...
const Form = () => {
  ...
  const calculateAmortization = () => {
    ...
    let paymentFrequency = frequencyMap[repaymentFrequency] || 12; // Default to Monthly
  
    // Adjust ongoing deposit based on payment frequency
    let modifiedOngoingDeposit = 0;
    if (repaymentFrequency === 'Monthly') {
      modifiedOngoingDeposit = ongoingDeposit;
    } else {
      modifiedOngoingDeposit = ongoingDeposit * 12 / paymentFrequency;
    }

  
    let totalPayments = loanTerm * paymentFrequency; // Convert years to total payments
    
    let periodicRate = annualInterestRate / 100 / paymentFrequency; // Adjust interest rate per period
    
    // Standard amortization formula adjusted for frequency
    let payment = (loanAmount * periodicRate) / (1 - Math.pow(1 + periodicRate, -totalPayments));
    ...
    for (let period = 1; period <= totalPayments; period++) {
      ...
      let interestWithOffset = (balanceWithOffset - tempOffsetAccountBalance) * periodicRate; // Adjusted for offset
      ...
      console.log(balanceWithOffset);    
      // Update offset account (if ongoing deposits are made)
      tempOffsetAccountBalance += modifiedOngoingDeposit;
  
      // Convert period count to months correctly
      let monthEquivalent = Math.floor(period / (paymentFrequency / 12)) + 1;
      ...
      if (balanceWithOffset === 0) {
        ...
        monthsSaved++;
        setYearsSaved(monthsSaved / frequencyMap[repaymentFrequency]);
      }
        ...
        <Grid container spacing={4}  padding={0} width={'105%'} >
          ...
          <Grid item xs={12} sm={12} md={12} lg={12}>
            <Flex gap={'small'} vertical>
              ...
              <InputNumber
                placeholder="Extra Monthly Repayment"
                defaultValue={ongoingDeposit}
                formatter={(value) => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                parser={(value) => value?.replace(/\$\s?|(,*)/g, '')}
